...
- (id)initWithManager:(SeaPlugins *)manager
{
	if (self = [super init]) {
		...
		[gOurBundle loadNibNamed:@"CIEdges" owner:self topLevelObjects:&tmpArray];
...
- (void)run
{
	...
	if ([defaults objectForKey:@"CIEdges.intensity"])
		self.intensity = [defaults floatForKey:@"CIEdges.intensity"];
	else
		self.intensity = 1.0;
	refresh = YES;
...
- (unsigned char *)edges:(PluginData *)pluginData withBitmap:(unsigned char *)data
{
	...
	//context = [[NSGraphicsContext currentContext] CIContext]; /* NOTE: Usually we would use the color-corrected context but in this case it seems to give duller results. */
	...
	size.width = width;
	size.height = height;
	//input = [CIImage imageWithBitmapData:[NSData dataWithBytesNoCopy:data length:width * height * 4 freeWhenDone:NO] bytesPerRow:width * 4 size:size format:kCIFormatARGB8 colorSpace:CGColorSpaceCreateDeviceRGB()];
