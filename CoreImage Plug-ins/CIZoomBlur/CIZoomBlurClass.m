...
#define CLASSMETHOD zoomBlur
#include "CICommon.mi"

- (unsigned char *)zoomBlur:(PluginData *)pluginData withBitmap:(unsigned char *)data
{
	CIContext *context;
	...
	if (opaque && [pluginData spp] == 4)
		backColor = [[CIColor alloc] initWithColor:[pluginData backColor:YES]];
	else if (opaque)
		backColor = [CIColor colorWithRed:[[pluginData backColor:YES] whiteComponent] green:[[pluginData backColor:YES] whiteComponent] blue:[[pluginData backColor:YES] whiteComponent]];
	...
	amount = (apoint.x - point.x) * (apoint.x - point.x) + (apoint.y - point.y) * (apoint.y - point.y);
	amount = sqrt(amount);
	amount = MIN(200, MAX(amount, 0));
