		...
		switch (propType) {
			...
			case PROP_COLORMAP:
				...
				if (version == 0) {
					...
					alert.messageText = LOCALSTR(@"indexed color title", @"Indexed colour not supported");
					alert.informativeText = LOCALSTR(@"indexed color body", @"XCF files using indexed colours are only supported if they are of version 1 or greater. This is because version 0 is known to have certain problems with indexed colours.");
					[alert runModal];
					return NO;
				}
				else {
...
- (instancetype)initWithDocument:(SeaDocument*)doc contentsOfURL:(NSURL *)path error:(NSError**)outError;
{
	...
	if (version > 2) {
		NSAlert *alert = [[NSAlert alloc] init];
		alert.messageText = LOCALSTR(@"xcf version title", @"XCF version not supported");
		alert.informativeText = LOCALSTR(@"xcf version body", @"The version of the XCF file you are trying to load is not supported by this program, loading may fail.");
		[alert runModal];
	}
